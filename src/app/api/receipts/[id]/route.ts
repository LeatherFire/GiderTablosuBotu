...
export async function GET(
  ...
  try {
    ...
    if (expense.receiptPath.startsWith('http://') || expense.receiptPath.startsWith('https://')) {
      return NextResponse.redirect(expense.receiptPath)
    }

    // Local dosya - eski sistem için geriye uyumluluk
    if (!existsSync(expense.receiptPath)) {
      return NextResponse.json({ error: 'Dekont dosyası bulunamadı' }, { status: 404 })
    }

    const fileBuffer = await readFile(expense.receiptPath)

    // MIME type belirleme
    let contentType = 'application/octet-stream'
    const ext = expense.receiptType || expense.receiptPath.split('.').pop()?.toLowerCase()
